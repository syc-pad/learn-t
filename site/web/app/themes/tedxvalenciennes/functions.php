		...
		<?php 
			foreach ($grandconference_options as $value) {
				...
				if($value['type'] == 'section')
				{
					...
					echo '<a id="pp_panel_'.strtolower($value['name']).'_a" href="#pp_panel_'.strtolower($value['name']).'" class="nav-tab '.esc_attr($active).'"><i class="fa '.esc_attr($value['icon']).'"></i>'.str_replace('-', ' ', $value['name']).'</a>';
				}
			}
		?>
		</h2>
	    ...
	    { 
	    	switch($_POST['demo'])
	    	{
		    	...
		    	case 2:
		    		...
		    		$page_on_front = 260; //Demo Homepage ID
